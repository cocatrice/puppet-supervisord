...
class supervisord::pip inherits supervisord {
  ...
  exec { 'upgrade_pip':
    command => 'pip install --index-url https://pypi.python.org/simple/ --upgrade "pip<20.3" && hash -r',
  ...
  exec { 'upgrade_setuptools':
    command     => 'pip install --upgrade setuptools==44.0.0 || pip list | grep "setuptools"',
